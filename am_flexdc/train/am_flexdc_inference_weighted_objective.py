...
- legacy weighted objective -> configurable FlexDC-component weights
- unconstrained P/R and weights -> current pilot feasibility bounds

The model predicts M_RSR, C_track, and C_Qos.  During inference, only
Pbar, R, and workload weights are updated by gradient descent.
"""
...
def load_context(results_csv, diagnostics_csv, workload, server_count, utilization, weight_sample_id):
    ...
    # Start from a real central pilot row, not an invented P/R point.
    # First take the median sampled Pbar, then the median valid R at that Pbar.
...
def calculate_optimization_objective(prediction, reserve, tracking_weight, qos_weight,
                                     reserve_penalty_per_kw_server):
    """Return the configured objective used only for input-gradient optimization."""
    return (
        prediction[0]
        + tracking_weight * prediction[1]
        + qos_weight * prediction[2]
        + reserve_penalty_per_kw_server * reserve
    )
...
def model_pr_descent(p_init,
                     ...
                     tracking_weight=1.0,
                     qos_weight=1.0,
                     reserve_penalty_per_kw_server=0.0,
                     min_pbar_minus_r=1e-6,
                     iterations=150,
                     lr=1e-3):
    ...
    if tracking_weight <= 0 or qos_weight <= 0:
        raise ValueError("tracking_weight and qos_weight must be positive.")
    if reserve_penalty_per_kw_server < 0:
        raise ValueError("reserve_penalty_per_kw_server must be non-negative.")
    if min_pbar_minus_r <= 0:
        raise ValueError("min_pbar_minus_r must be positive.")

    # Pbar must leave room for both the R lower bound and Pbar-R minimum.
    effective_p_lower = max(float(p_lower), float(r_lower) + float(min_pbar_minus_r))
    effective_p_upper = min(float(p_upper), float(pr_upper) - float(r_lower))
    if effective_p_lower > effective_p_upper:
        raise ValueError("P/R bounds are infeasible after applying the minimum Pbar-R margin.")
    ...
    for iteration in range(iterations + 1):
        ...
        objective = calculate_optimization_objective(
            prediction=prediction,
            reserve=reserve,
            tracking_weight=tracking_weight,
            qos_weight=qos_weight,
            reserve_penalty_per_kw_server=reserve_penalty_per_kw_server,
        )
        ...
        records.append({
            ...
            **{f"Weight_{i}": float(weights[i].detach()) for i in range(job_count)},
            ...
            "Predicted_C_total": float(predicted_total.detach()),
            "Optimization_Objective": float(objective.detach()),
        })
        ...
        with torch.no_grad():
            ...
            pbar.clamp_(effective_p_lower, effective_p_upper)
            reserve_upper = min(
                float(pbar) - float(min_pbar_minus_r),
                float(pr_upper) - float(pbar),
            )
            reserve.clamp_(float(r_lower), reserve_upper)
...
def weighted_objective_from_components(components, reserve, tracking_weight, qos_weight,
                                        reserve_penalty_per_kw_server):
    return float(
        components[0]
        + tracking_weight * components[1]
        + qos_weight * components[2]
        + reserve_penalty_per_kw_server * reserve
    )
...
def parse_args():
    ...
    # Optional operational objective weights. Defaults preserve the original current-model objective.
    parser.add_argument("--tracking-weight", type=float, default=1.0,
                        help="Multiplier on predicted C_track in the optimization objective.")
    parser.add_argument("--qos-weight", type=float, default=1.0,
                        help="Multiplier on predicted C_Qos in the optimization objective.")
    parser.add_argument("--reserve-penalty-per-kw-server", type=float, default=0.0,
                        help="Additional penalty on R in cost units per kW/server. This reduces the incentive to maximize R.")
    parser.add_argument("--min-pbar-minus-r", type=float, default=1e-6,
                        help="Strict lower bound on Pbar-R in kW/server. Default keeps the original feasibility rule R < Pbar.")
...
def main():
    ...
    trajectory = model_pr_descent(
        ...
        reserve_penalty_per_kw_server=args.reserve_penalty_per_kw_server,
        min_pbar_minus_r=args.min_pbar_minus_r,
    ...
    optimized = trajectory.loc[trajectory["Optimization_Objective"].idxmin()]
    ...
    best_objective = weighted_objective_from_components(
        best_prediction,
        float(best_sampled["R_kw_per_server"]),
        args.tracking_weight,
        args.qos_weight,
        args.reserve_penalty_per_kw_server,
    )
    best_all_objective = weighted_objective_from_components(
        best_all_prediction,
        float(best_sampled_all["R_kw_per_server"]),
        args.tracking_weight,
        args.qos_weight,
    ...
    comparison = pd.DataFrame([
        ...
        {
            "Configuration": "Lowest actual raw-objective equal-weight pilot point",
            "Weight_Sample_ID": int(args.weight_sample_id),
        ...
        {
            "Configuration": "Lowest actual raw-objective pilot point (all accepted weights)",
            ...
            **dict(zip(weight_names, best_all_weights)),
            ...
            "Optimization_Objective": best_all_objective,
    ...
    summary = {
        ...
        "optimization_objective": float(optimized["Optimization_Objective"]),
        "tracking_weight": args.tracking_weight,
        "qos_weight": args.qos_weight,
        "reserve_penalty_per_kw_server": args.reserve_penalty_per_kw_server,
        "min_pbar_minus_r": args.min_pbar_minus_r,
