"""Training utilities for the CONDOR-based FlexDC behavior model (v2).

Key improvements over v1
------------------------
* physics-derived per-job and global features, standardized on training only;
* context-stratified grouped splits with seed replicas kept together;
...
def build_training_sampling_weights(
    ...
    *,
    ...
    if mode in {"natural", "shuffle", "none"}:
        return None, {"mode": "natural"}
    if mode != "balanced":
        raise ValueError(f"Unknown sampler mode: {mode}")
    fractions = np.asarray([natural_fraction, context_fraction, priority_fraction], dtype=float)
    if np.any(fractions < 0) or not np.isclose(fractions.sum(), 1.0, atol=1e-8):
        raise ValueError("natural/context/priority sampler fractions must be nonnegative and sum to 1")

    n = len(train_df)
    natural = np.full(n, 1.0 / max(n, 1), dtype=float)

    context_counts = train_df["_context_id"].value_counts()
    num_contexts = len(context_counts)
    context = np.asarray(
        [1.0 / (num_contexts * context_counts[context_id]) for context_id in train_df["_context_id"]],
        dtype=float,
    )

    priority_score = np.ones(n, dtype=float)
    priority_score *= np.where(train_df["_actual_both_pass"].to_numpy(bool), feasible_boost, 1.0)
    priority_score *= np.where(train_df["_tracking_boundary"].to_numpy(bool), tracking_boundary_boost, 1.0)
    priority_score *= np.where(train_df["_qos_boundary"].to_numpy(bool), qos_boundary_boost, 1.0)
    priority = priority_score / priority_score.sum()

    probabilities = (
        natural_fraction * natural
        + context_fraction * context
        + priority_fraction * priority
    )
    probabilities = probabilities / probabilities.sum()

    expected_feasible_share = float(probabilities[train_df["_actual_both_pass"].to_numpy(bool)].sum())
    expected_tracking_boundary_share = float(probabilities[train_df["_tracking_boundary"].to_numpy(bool)].sum())
    expected_qos_boundary_share = float(probabilities[train_df["_qos_boundary"].to_numpy(bool)].sum())
    audit = {
        "mode": "balanced",
        "natural_fraction": float(natural_fraction),
        ...
        "raw_feasible_share": float(train_df["_actual_both_pass"].mean()),
        "expected_sampled_feasible_share": expected_feasible_share,
        "raw_tracking_boundary_share": float(train_df["_tracking_boundary"].mean()),
        "expected_sampled_tracking_boundary_share": expected_tracking_boundary_share,
        "raw_qos_boundary_share": float(train_df["_qos_boundary"].mean()),
...
def prepare_behavior_data(
    *,
    ...
    train_groups, heldout_groups, split_audit = stratified_context_group_split(
        merged,
        heldout_fraction=heldout_fraction,
        split_seed=split_seed,
    )
    train_df = merged[merged["_group_id"].isin(train_groups)].copy().reset_index(drop=True)
    heldout_df = merged[merged["_group_id"].isin(heldout_groups)].copy().reset_index(drop=True)
    ...
    audit = {
        ...
        **split_audit,
        ...
        "heldout_actual_feasible_rows": int(heldout_df["_actual_both_pass"].sum()),
        "sampler": sampler_audit,
    }
    return BehaviorDataBundle(
        dataframe=merged,
        train_dataframe=train_df,
        heldout_dataframe=heldout_df,
...
def _load_training_checkpoint(path: str | Path, *, model, optimizer, device: torch.device) -> dict:
    ...
    if int(checkpoint.get("format_version", 0)) != 2:
        raise ValueError("Resume requires a v2 training checkpoint.")
...
def train_behavior_model(
    ...
    *,
    ...
    checkpoint_prefix: str = "flexdc_behavior_v2",
    ...
    if metrics_every_n_epochs != 1:
        raise ValueError("v2 checkpoint/early-stopping logic requires metrics_every_n_epochs=1")
    ...
    best_scores = {
        ...
        "heldout_objective_r2": -float("inf"),
        "heldout_feasibility_f1": -float("inf"),
    }
    ...
    if resume_from:
        ...
        best_scores.update(checkpoint.get("best_scores", {}))
    ...
    for epoch in range(start_epoch, epochs):
        ...
        heldout_loss = float(heldout_metrics["heldout/loss/total"])
        objective_r2 = float(heldout_metrics["heldout/cost/full_objective/r2"])
        feasibility_f1 = float(heldout_metrics["heldout/feasibility/combined/f1_feasible"])
        improved_loss = heldout_loss < best_scores["heldout_loss"] - early_stopping_min_delta
        improved_objective = np.isfinite(objective_r2) and objective_r2 > best_scores["heldout_objective_r2"]
        improved_feasibility = np.isfinite(feasibility_f1) and feasibility_f1 > best_scores["heldout_feasibility_f1"]
        if improved_loss:
            best_scores["heldout_loss"] = heldout_loss
            best_epochs["best_loss"] = epoch
            epochs_without_improvement = 0
        else:
            epochs_without_improvement += 1
        if improved_objective:
            best_scores["heldout_objective_r2"] = objective_r2
            best_epochs["best_objective"] = epoch
        if improved_feasibility:
            best_scores["heldout_feasibility_f1"] = feasibility_f1
        ...
        if verbose:
            print(
                ...
                f"heldout={heldout_loss:.6g} | "
                f"feas P/R={heldout_metrics['heldout/feasibility/combined/feasible_precision']:.4f}/"
                f"{heldout_metrics['heldout/feasibility/combined/actual_feasible_accuracy']:.4f} | "
        ...
        if epochs_without_improvement >= early_stopping_patience:
            ...
            if verbose:
                print(
                    f"Early stopping at epoch {epoch}; best heldout loss was "
                    f"epoch {best_epochs['best_loss']} ({best_scores['heldout_loss']:.6g})."
                )
            break
...
def final_behavior_evaluation(
    ...
    *,
    ...
    assert train_rows is not None and heldout_rows is not None
